...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Admin Settings - Get API Keys
    if (endpoint === '/settings/api-keys' && method === 'GET') {
      // Return current API key configuration (without exposing actual keys)
      return {
        statusCode: 200,
        headers: corsHeaders,
        body: JSON.stringify({
          apiKeys: {
            ...
            elevenlabsApiKey: process.env.ELEVENLABS_API_KEY ? '***configured***' : null,
            sendgridApiKey: process.env.SENDGRID_API_KEY ? '***configured***' : null,
            awsAccessKey: process.env.AWS_ACCESS_KEY_ID ? '***configured***' : null,
            awsSecretKey: process.env.AWS_SECRET_ACCESS_KEY ? '***configured***' : null,
            stripePublishableKey: process.env.STRIPE_PUBLISHABLE_KEY ? '***configured***' : null,
            stripeSecretKey: process.env.STRIPE_SECRET_KEY ? '***configured***' : null
          }
        })
      };
    }

    // Admin Settings - Update API Keys
    if (endpoint === '/settings/api-keys' && method === 'POST') {
      const requestBody = JSON.parse(event.body || '{}');
      const { apiKeys } = requestBody;

      // For security, we can't actually update environment variables from the function
      // This would typically integrate with a secure configuration management system
      // For now, return success but log that manual environment variable updates are needed
      
      console.log('API Keys update requested by admin:', {
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          message: 'API keys configuration saved. Note: Server environment variables may need manual update for full effect.',
          updatedKeys: Object.keys(apiKeys || {})
        })
      };
    }

    // Admin Settings - Get General Settings
    ...
    if (endpoint === '/settings' && method === 'POST') {
      ...
      console.log('General settings update requested by admin:', {
        userId: decoded.userId,
        email: decoded.email,
        timestamp: new Date().toISOString(),
